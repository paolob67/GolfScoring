...
async function refreshLogInStatus() {
  ...
  if (user) {
    localStorage.setItem('gsUserName', user.name);
    localStorage.setItem('gsUserId', user.id);
    localStorage.setItem('gsRoles', user.roles);
    applyLoggedInUi(user);
    // await updateCartDetails();
  } else {
    localStorage.removeItem('gsToken');
    localStorage.removeItem('gsUserId');
    localStorage.removeItem('gsUserName');
    localStorage.removeItem('gsRoles');
    applyLoggedOutUi();
  }
}
/*
...
function showProfile() {
  const userId = localStorage.getItem('gsUserId');
